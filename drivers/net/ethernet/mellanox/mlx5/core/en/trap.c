...
static int mlx5e_open_trap(struct mlx5e_priv *priv)
{
...
void mlx5e_close_trap(struct mlx5e_priv *priv)
{
...
static void mlx5e_activate_trap(struct mlx5e_priv *priv)
{
	struct mlx5e_trap *trap = priv->en_trap;
...
static int mlx5e_add_trap_queue(struct mlx5e_priv *priv)
{
	int err;

	err = mlx5e_open_trap(priv);
	if (err)
		return err;

	mlx5e_activate_trap(priv);
	return 0;
}
