...
static int
mlx5_ipsec_esp_validate_xfrm_attrs(struct mlx5_core_dev *mdev,
			   ...
			   u32 flags)
{
	...
	err = mlx5_ipsec_esp_validate_xfrm_attrs(mdev, attrs);
	if (err)
		return ERR_PTR(err);
...
static void mlx5_ipsec_release_sa_ctx(struct mlx5_ipsec_sa_ctx *sa_ctx)
{
...
{
	struct mlx5_ipsec_esp_xfrm *mxfrm = container_of(xfrm, struct mlx5_ipsec_esp_xfrm, accel_xfrm);
